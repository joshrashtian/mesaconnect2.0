    ...
    <main className="gap-3 flex flex-col">
        ...
        {preview &&
          preview.map((e: string, index: number) => {
            return (
              <div
                key={index}
                className="bg-slate-white shadow-lg text-xl focus:outline-none hover:shadow-sm hover:scale-105 duration-300 h-12 rounded-full px-5 p-3"
              >
                {e}
              </div>
            )
      ...
      <button
        ...
        className={`p-3 w-full rounded-full ${
          !preview || preview.length < 2 ? 'bg-slate-200' : 'bg-orange-500 text-white'
        } duration-300 `}
      >
        <p>Submit</p>
      </button>
