...
import { IoBook, IoCode, IoFlask, IoGameController, IoPencil } from 'react-icons/io5'
...
export const NewInterests = ({ userinterests }: { userinterests: Interest[] | undefined }) => {
  const [interests, setInterests] = useState<Interest[]>()
  const [custom, setCustom] = useState(false)
  const [dropdownCustom, setDropdownCustom] = useState(false)
  const [category, setCategory] = useState<string>()
  const [toAdd, setToAdd] = useState<Interest[]>()
  const { user } = useUser()
  useEffect(() => {
    const getInterests = async () => {
      const { data, error } = await supabase.from('availableinterests').select('*').limit(10)
  ...
  return (
    <div className="font-eudoxus flex flex-col gap-5">
      ...
      <menu className="flex flex-col gap-0.5 h-full overflow-y-scroll">
        <motion.ul
        ...
        >
          <li className="flex gap-3 items-center ">
            <IoPencil
              className={`${custom && 'scale-0 translate-y-12'} duration-700 origin-bottom`}
            />
            <h1
              className={`${
          ...
          <h2 className={`text-slate-400 duration-500 ${custom && 'scale-0  translate-y-12'}`}>
            Click To Create
          </h2>
          <motion.ul
            onClick={(e) => e.preventDefault()}
            className={` scale-0 flex justify-center items-center gap-3 absolute translate-x-1 -translate-y-2 duration-500 ${
              custom && 'scale-100 translate-y-12'
            }`}
          >
            <input className="p-1 px-4 hover:scale-[1.02] duration-300 focus:bg-slate-50 outline-none bg-white rounded-xl shadow-sm " />
            <button
            ...
            >
              <IoMdArrowDropdown
                className={`${dropdownCustom ? 'rotate-90' : ' rotate-0'} duration-300`}
              />
            </button>
            <AnimatePresence>
              {dropdownCustom && (
