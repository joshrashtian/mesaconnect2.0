...
const PollModal = ({ data, disarm }: { data: PollType; disarm: () => void }) => {
  ...
  useEffect(() => {
    ...
    if (data.context) {
      setContext(
        `https://gnmpzioggytlqzekuyuo.supabase.co/storage/v1/object/public/questionContexts/${data.id}`
      )
      setLoaded(true)
    } else setLoaded(true)
  }, [])
  ...
  return (
    <main className="p-12  flex flex-col justify-between h-full">
      ...
      >
        ...
        {context && <img src={context} className="w-full h-full object-contain" />}
      </section>
