...
const ProfilePage = ({ params }: { params: { id: string } }) => {
  ...
  const BioComp = useCallback(() => <BioModal bio={user?.bio} user={ActiveUser.user?.id} />, [user])
  ...
  return (
    ...
    >
      <ul className="flex flex-row items-end gap-5 ">
        ...
        <ul>
          <h1 className="font-black text-6xl font-eudoxus dark:text-white/70">{user?.real_name}</h1>
      ...
      <h2
        ...
        className={`dark:text-white ${
          isActiveUser && 'hover:bg-slate-200/60 p-3 duration-300 cursor-pointer'
        }`}
      >
