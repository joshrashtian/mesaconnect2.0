...
const EditWidgets = ({ current }: { current: any }) => {
  ...
  const updateWidgets = async (e: any) => {
    ...
    const { data, error } = await supabase
      .from('profiles')
      .update({ widgets: current ? [...current, ...NewWidget] : [...NewWidget] })
      .eq('id', user.user?.id)
  ...
  return (
    <motion.main
      ...
      className="absolute bottom-0 w-full h-1/3 rounded-t-3xl flex flex-col justify-center gap-2 px-10 overflow-x-scroll shadow-inner opacity-80 bg-zinc-100"
    >
