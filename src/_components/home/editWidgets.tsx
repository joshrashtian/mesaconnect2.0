  ...
  return (
    <>
      <motion.main
        ...
        className="absolute bottom-10 font-eudoxus left-[10%] dark:bg-zinc-700 w-[80%] h-1/4 resize min-h-1/4 rounded-3xl flex flex-col justify-center gap-2 p-10 px-10 overflow-x-scroll no-scrollbar shadow-2xl opacity-80 bg-zinc-100"
      >
        <h1 className="font-bold text-xl dark:text-slate-100">Add Widgets</h1>
          ...
          {widgets.map((widget) => {
            return (
              <div key={widget.name} className="flex flex-col gap-0.5 items-center">
                ...
                <p className="text-white">{widget.name}</p>
              </div>
            );
          })}
        </section>
